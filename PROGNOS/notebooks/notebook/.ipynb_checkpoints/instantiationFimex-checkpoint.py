...
#     fabfile = 'fabfile.py'
    ...
    def __init__(self,instance,username,region,keyDir,machineType):
        ...
        fimex.system = platform.system()
        if (fimex.system == 'Windows'):
            fimex.options['shell'] = True
    ...
    def callPopen(self,verbose=True,overwrite=False):
        ...
        with Popen(cmd.split(),**fimex.options) as p:
    ...
    def isInstance(self):
        self.instanceExists=False
        self.ip=''
        with Popen('gcloud compute instances list'.split(),**fimex.options) as p:
            for line in p.stdout:
