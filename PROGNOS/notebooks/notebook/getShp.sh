...
shp2pgsql -s 3035 basin$id.shp $schema.dummy$id | PGPASSWORD="$password" psql -U "$user" -d geonorway -h localhost -p 5432 -q 
...
read -r -d '' SQL <<- EOM
    ...
    FROM $schema.stations AS b, $schema.dummy$id AS a
    WHERE b.station_id=$id
    GROUP BY station_id, station_name;
    DROP TABLE $schema.dummy$id; 
...
echo "You are lucky to be alive!"
